from seg.modules import DinoFeaturizer
from seg.dataset import MVTecLocoDataset
from torch.utils.data import DataLoader
import torch
from seg.sampler import GreedyCoresetSampler
import torch.nn.functional as F
import cv2
import numpy as np
import os
from sklearn.cluster import KMeans
from seg.crf import dense_crf
...
def run() -> None:
    ...
    net = DinoFeaturizer()
    net = net.cuda()
    netup = torch.hub.load("mhamilton723/FeatUp", 'dinov2', use_norm=use_norm)
    netup = netup.cuda()
    train_feature_list=[]
    ...
    if StartTrain:
        for i, Img in enumerate(dataloader):
            with torch.no_grad():
                ...
                feats2 = netup.model(image)
                feats4 = netup(image)
                feats = torch.cat((torch.nn.functional.interpolate(feats2, (28, 28), mode="nearest"), feats0),
                                  dim=1)
                feats = torch.cat((torch.nn.functional.interpolate(feats4, (28, 28), mode="nearest"), feats),
                                  dim=1).squeeze()

                # feats = feats0.squeeze()
                feats = feats.reshape(feats.shape[0],-1).permute(1,0)
    ...
    save_img(dataloader, train_features_sampled, net, netup, train_savepath)

    test_savepath = f'{savepath}/test/logical_anomalies'
    if not os.path.exists(test_savepath):
        os.makedirs(test_savepath)
    save_img(dataloader_test_logical, train_features_sampled, net, netup, test_savepath)

    test_savepath = f'{savepath}/test/good'
    if not os.path.exists(test_savepath):
        os.makedirs(test_savepath)
    save_img(dataloader_test_good, train_features_sampled, net, netup, test_savepath)

    test_savepath = f'{savepath}/test/stru'
    if not os.path.exists(test_savepath):
        os.makedirs(test_savepath)
    save_img(dataloader_test_stru, train_features_sampled, net, netup, test_savepath)

def save_img(dataloader, train_features_sampled, net, netup, savapath):
    for i, Img in enumerate(dataloader):
        image = Img['image']
        imageo = Img['image1'][0,:,:,:]
        imageo = unloader(imageo)
        heatmap,heatmap_intra = get_heatmaps(image, train_features_sampled, net, netup)
...
def get_heatmaps(img,query_feature,net,netup):
    with torch.no_grad():
        feats1, f1_lowdim = net(img.cuda())
        feats3 = netup.model(img.cuda())
        feats5 = netup(img.cuda())
        feats = torch.cat((torch.nn.functional.interpolate(feats3, (28, 28), mode="nearest"), feats1), dim=1)
        feats1 = torch.cat((torch.nn.functional.interpolate(feats5, (28, 28), mode="nearest"), feats), dim=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datasetpath', action='store', default='./mvtec_loco_anomaly_detection')
    args = parser.parse_args()

    unloader = transforms.ToPILImage()
    StartTrain = True
    image_size = 224
    color_list = [[127, 123, 229], [195, 240, 251], [146, 223, 255], [243, 241, 230], [224, 190, 144], [178, 116, 75], [128, 0, 128]]
    ...
    classlist = ['breakfast_box']   #'juice_bottle', 'pushpins', 'splicing_connectors', 'screw_bag',
